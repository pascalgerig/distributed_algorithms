	...
	\begin{enumerate}[a)]
		...
		\begin{tabular}{ccccc}
			\begin{tabular}{|p{40em}|}
				...
				\ \ \ trigger  $\langle \textit{pl, send} \mid \ p_i, \ $[\textit{WRITE, ts', } $(i, pol(v_i))] \rangle$\\
				\\
		...
		For this reason a regular register has to guarantee that all the neede processes will either return the last written or the concurrently written value, bot all the processes need to return the same of the two options.
